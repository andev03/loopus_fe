...
export default function ChatDetailScreen() {
    ...
    if (newPoll) {
      ...
      if (poll.options && Array.isArray(poll.options)) {
        poll.options = (poll.options || []).map((o, i) => {
          ...
          return {
            optionId:
              o.optionId || o.id || o._id || o.option?.optionId || `temp-${i}`,
            text: o.optionText || o.text || o.name || "Không tên",
            votes: Array.isArray(o.votes) ? o.votes : [],
          };
        });
      ...
      poll.sender =
        poll.createdBy?.fullName || poll.creator?.fullName || "Thành viên";
      poll.senderId = poll.createdBy?.userId || poll.creator?.userId;
      poll.avatarUrl =
        poll.createdBy?.avatarUrl ||
  ...
  const takePhoto = async () => {
    ...
    if (permissionResult.granted === false) {
      Alert.alert("Quyền bị từ chối", "Cần cấp quyền để sử dụng máy ảnh!");
      return;
    }
    ...
    let result = await ImagePicker.launchCameraAsync({
      ...
      quality: 1,
      allowsEditing: true, // Cho phép chỉnh sửa ảnh sau khi chụp (tùy chọn)
    });

    if (!result.canceled) {
      const image = result.assets[0];
      setSelectedImage(image);
      handleSendImage(image); // Gửi ảnh ngay sau khi chụp
    }
  };
        ...
        if (res.success && Array.isArray(res.data) && res.data.length > 0) {
          const polls = res.data.map((p) => {
            ...
            return {
              ...
              sender:
                p.createdBy?.fullName || p.creator?.fullName || "Thành viên",
              ...
              isCurrentUser:
                user?.userId === (p.createdBy?.userId || p.creator?.userId),
            };
          });
  ...
  const fetchEventParticipants = async (eventId) => {
    ...
    try {
      ...
      const acceptedRes = await eventService.getEventParticipants(
        eventId,
        "ACCEPTED"
      );
      const declinedRes = await eventService.getEventParticipants(
        eventId,
        "DECLINED"
      );
      ...
      const uniqueAccepted = acceptedUsers.filter(
        (user, index, self) =>
          index === self.findIndex((u) => getUserId(u) === getUserId(user))
      );

      const uniqueDeclined = declinedUsers.filter(
        (user, index, self) =>
          index === self.findIndex((u) => getUserId(u) === getUserId(user))
      );

      const uniqueAll = allUsers.filter(
        (user, index, self) =>
          index === self.findIndex((u) => getUserId(u) === getUserId(user))
      );
      ...
      <View style={{ marginVertical: 15 }}>
        ...
        <View style={{ marginBottom: 15 }}>
          ...
          {participants.accepted.length === 0 ? (
            <Text
              style={[styles.eventModalText, { color: "#999", fontSize: 14 }]}
            >
      ...
      { text: "Hủy", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            ...
            if (res.success) {
              setMessages((prev) =>
                prev.filter((m) => String(m.id) !== String(pollId))
              );
              await AsyncStorage.setItem(
                `messages_${groupId}`,
                JSON.stringify(
                  messages.filter((m) => String(m.id) !== String(pollId))
                )
              );
  ...
  const renderMessage = ({ item }) => {
    ...
    if (item.type === "poll") {
      ...
      const totalVotes = safeOptions.reduce(
          ...
          <View style={styles.pollCard}>
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                console.log(
                  "🟢 [FINAL] safePoll.options:",
                  JSON.stringify(safePoll.options, null, 2)
                );
  ...
  const currentEventStatus = selectedEvent
    ? eventStatuses[selectedEvent.eventId]
    : null;
...
>
  {/* Bắt chạm ngoài menu để đóng menu */}
    ...
    <View style={{ flex: 1 }}>
      ...
      {!isSearchMode && (
        <SafeAreaView edges={["bottom"]} style={{ backgroundColor: "#fff" }}>
          ...
          {/* MENU: Dùng pointerEvents="box-none" để không bị chặn chạm */}
          {showMenu && (
        ...
        <View style={styles.addOptionRow}>
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              ...
              try {
                const res = await pollService.addOption(
                  selectedPoll.id,
                  newOption.trim()
                );
                if (res.success) {
                  const newOpt = {
                    ...
                    text: res.data.optionText || newOption.trim(),
                    votes: Array.isArray(res.data.votes) ? res.data.votes : [],
                  };
                  setMessages((prev) => {
                    const updated = prev.map((m) =>
                      String(m.id) === String(selectedPoll.id)
                        ? { ...m, options: [...m.options, newOpt] }
                        : m
                    );
                    AsyncStorage.setItem(
                      `messages_${groupId}`,
                    ...
                    const newSelected = updated.find(
                      (m) => String(m.id) === String(selectedPoll.id)
                    );
              ...
              {user && (
                <View style={styles.confirmationSection}>
                  {currentEventStatus && (
                    ...
                    >
                      <Text style={styles.eventModalText}>
                        Bạn xác nhận: {currentEventStatus}
                      </Text>
                      <Ionicons
                  ...
                  <View style={styles.eventActions}>
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        handleConfirmEvent(selectedEvent.eventId, "Từ chối")
                      }
                    >
                      <Ionicons
                        name={
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        handleConfirmEvent(selectedEvent.eventId, "Tham gia")
                      }
                    >
                      <Ionicons
                        name={
